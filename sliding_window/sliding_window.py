...
class SlidingWindow:
    def __init__(self, path_to_data, target, window_size, n_largest, stride):
        ...
        self.fasta_extensions = (".fasta", ".fa", ".fna", ".ffn", ".faa", ".frn")
    ...
    def subset_summary_stats(self, is_target):
        ...
        return pd.DataFrame(
            [[alignment_length, seq_count, target_count, proportion]],
            columns=[
                "alignment_length",
                "sequence_count",
                "target_count",
    ...
    def add_header(self, header, window_frequencies):
        ...
        window_frequencies = pd.DataFrame(window_frequencies, columns=columns)
        return pd.concat([header, window_frequencies], axis=1)

    # returns a dataframe of window frequencies for each sequence in a subset
    ...
    def make_summary_stats(self):
        ...
        subsets = [
            self.frequencies.loc[self.frequencies["subset"] == subset]
    ...
    def concat_dataframes(self, deltas, effect_sizes):
        self.window_data = pd.concat([self.window_data, deltas, effect_sizes], axis=1)
    ...
    def calculate_effect_sizes(self, deltas):
        ...
        return [
            self.calculate_hedges_g(delta, group[1], sample_size, sample_size_sums)
    ...
    def make_effect_sizes(self):
        ...
        effect_sizes = self.calculate_effect_sizes(deltas)
        deltas = pd.DataFrame(
            np.concatenate(deltas), columns=self.make_column_names("_delta")
        )
        effect_sizes = pd.DataFrame(
            np.concatenate(effect_sizes), columns=self.make_column_names("_hedges_g")
        )
    ...
    def filter_dataframe(self, array):
        indices = np.argpartition(array, -self.n_largest)[-self.n_largest :] + 1
        mask = self.window_data["window"].isin(indices)
        self.window_data_filtered = self.window_data[mask].copy().reset_index(drop=True)
    ...
    def make_window_start(self):
        ...
        return [
            self.interpolate_range(value, old_min, old_max, 1, alighment_length)
            for value in self.window_data["window"]
        ]

    def make_window_data(self):
        groupby_subset = self.frequencies.groupby("subset")
        self.window_data = groupby_subset.mean(numeric_only=True).T.stack().reset_index()
        variance = groupby_subset.var(numeric_only=True).T.stack().reset_index(drop=True)
